...
def restore(require_confirm=True, file="watcher.zip"):
    ...
    if not os.path.isfile(file):
        print(
            "watcher.zip not found. Place watcher.zip in same directory as backup script."
        )
        return

    if require_confirm is True:
        ans = input(
            "Restoring backup. This will overwrite existing "
...
if __name__ == "__main__":
    ...
    group.add_argument(
        "-b", "--backup", help="Back up to watcher.zip.", action="store_true"
    )
    group.add_argument(
        "-r", "--restore", help="Restore from watcher.zip.", action="store_true"
    )
    group.add_argument(
        "-y",
        "--confirm",
        help="Ignore warnings and answer Y to prompts.",
        action="store_true",
    )
