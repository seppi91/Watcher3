...
class Torrent(NewzNabProvider):
    ...
    def search_all(self, imdbid, title, year):
        ...
        for indexer in torz_indexers:
            ...
            if "imdbid" in caps:
                ...
                r = self.search_newznab(
                    url_base, apikey, "search", q=term, imdbid=imdbid
                )
                if not r and no_year:
                    logging.info(
                        "{} does not find anything, trying without year, using q={}".format(
                            url_base, title
                        )
                    )
                    r = self.search_newznab(
                        url_base, apikey, "search", q=title, imdbid=imdbid
                    )
            for i in r:
                results.append(i)
        ...
        for indexer, enabled in core.CONFIG["Indexers"]["Torrent"].items():
            if enabled:
                if not hasattr(torrent_modules, indexer):
                    logging.warning(
                        "Torrent indexer {} enabled but not found in torrent_modules.".format(
                            indexer
                        )
                    )
                    continue
    ...
    def _get_caps(self, url_base, apikey):
        ...
        try:
            ...
            caps = yahoo.data(fromstring(xml))["caps"]["searching"]["movie-search"][
                "supportedParams"
            ]
