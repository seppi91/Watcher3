...
def get_rss():
    ...
    try:
        url = "https://www.torrentapi.org/pubapi_v2.php?token={}&mode=list&category=movies&format=json_extended&app_id=Watcher".format(
            _token()
        )
        timeout = datetime.datetime.now() + datetime.timedelta(seconds=2)
...
class RarbgTokenError(Exception):
    ...
    def __init__(self, msg=None):
        self.msg = (
            msg if msg else "Failed to retrieve new token from Rarbg's torrentapi.org."
        )
