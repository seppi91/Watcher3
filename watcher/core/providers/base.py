...
class NewzNabProvider(object):
    ...
    def _get_rss(self):
        ...
        for indexer in indexers:
            ...
            url = "{}api?t=movie&cat=2000&extended=1&offset=0&apikey={}".format(
                url_base, apikey
            )

            logging.info(
                "RSS_SYNC: {}api?t=movie&cat=2000&extended=1&offset=0&apikey=APIKEY".format(
                    url_base
                )
            )
        ...
        for item in items:
            try:
                ...
                result = {
                    ...
                    "downloadid": None,
                    "freeleech": float(item["attr"].get("downloadvolumefactor", 1))
                    == 0.0,
                    "guid": guid,
                    "indexer": indexer,
                    ...
                    else "tt{}".format(item["attr"].get("imdb")),
                    "pubdate": item.get("pubDate", "")[5:16],
                    "score": 0,
                    "seeders": 0,
                    "size": int(
                        item.get("size") or item.get("enclosure", {}).get("length", 0)
                    ),
                    "status": "Available",
    ...
    def test_connection(indexer, apikey):
        ...
        if e_code:
            if error_json["error"].get("description") == "Missing parameter":
                ...
                logging.error(
                    "Newz/TorzNab connection test failed. {}".format(
