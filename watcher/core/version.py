    ...
    def update_check(self, add_notif=True, install=True):
        ...
        if data["status"] == "error":
            ...
            compare = "{}/compare/{}...{}".format(
                core.GIT_URL, data["local_hash"], data["new_hash"]
            )

            notif = {
                "type": "update",
                "title": title,
                "message": 'Click <a onclick="_start_update(event)"><u>here</u></a> to update now.<br/> Click <a href="{}" target="_blank" rel="noopener"><u>here</u></a> to view changes.'.format(
                    compare
                ),
            }
            ...
            if install and core.CONFIG["Server"]["installupdates"]:
                ...
                logging.info(
                    "Currently {} commits behind. Automatic install disabled".format(
                        core.UPDATE_STATUS["behind_count"]
                    )
                )
...
class GitUpdater(UpdateBase):
    ...
    def _update_check(self):
        ...
        if self.current_hash[2] == 1:
            logging.error(
                "Error getting local commit hash: {}".format(self.current_hash[1])
            )
            result["status"] = "error"
        ...
        if commit_history[2] == 1:
            logging.error(
                "Error getting git commit history: {}".format(commit_history[1])
            )
            result["status"] = "error"
        ...
        if local_hash in commit_list:
            ...
            # if not, find out how far behind we are
                ...
                logging.debug(
                    "{} updates are available -- latest commit: {}.".format(
                        behind_count, commit_list[0]
                    )
                )
...
class ZipUpdater(UpdateBase):
    ...
    def _update_check(self):
        ...
        if not newest_hash:
            ...
            result[
                "error"
            ] = "Could not get latest update hash. Check logs for details."
            core.UPDATE_STATUS = result
            return result
