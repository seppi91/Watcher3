...
def sync():
    ...
    for k, v in core.CONFIG["Search"]["Watchlists"]["Traktlists"].items():
        ...
        movies += [
            i
            for i in get_list(k, min_score=min_score, length=length)
            if i not in movies
        ]
    ...
    for i in movies:
        ...
        try:
            if (
                ...
                searcher.search(
                    imdbid, i["title"], i["year"], core.config.default_profile()
                )
        except Exception as e:
            logging.error(
                "Movie {} did not get added.".format(i["title"]), exc_info=False
            )
    return success
...
def sync_rss():
    ...
    for url in core.CONFIG["Search"]["Watchlists"]["traktrss"].split(","):
        ...
        logging.info(
            "Syncing Trakt RSS watchlist {}. Last sync: {}".format(list_id, last_sync)
        )
        try:
        ...
        for entry in root.iter("entry"):
            try:
                pub = datetime.datetime.strptime(
                    entry.find("published").text[:19], trakt_date_format
                )
                if last_sync >= pub:
                    break
                else:
                    ...
                    logging.info(
                        "Searching TheMovieDatabase for {} {}".format(title, year)
                    )
                    movie = Manage.tmdb._search_title("{} {}".format(title, year))[0]
                    if movie:
                        movie["origin"] = "Trakt"
                        logging.info(
                            "Found new watchlist movie {} {}".format(title, year)
                        )
                        ...
                        logging.warning(
                            "Unable to find {} {} on TheMovieDatabase".format(
                                title, year
                            )
                        )
    ...
    if core.sql.row_exists("SYSTEM", name="trakt_sync_record"):
        core.sql.update(
            "SYSTEM", "data", json.dumps(record), "name", "trakt_sync_record"
        )
    else:
        core.sql.write(
            "SYSTEM", {"data": json.dumps(record), "name": "trakt_sync_record"}
