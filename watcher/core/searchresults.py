...
def score(releases, imdbid=None, imported=False):
    ...
    if imported:
        logging.debug(
            'Releases are of origin "Import", using custom default quality profile.'
        )
        titles = []
        check_size = False
        movie_details = {}
    ...
    required_groups = (
        words_to_list(quality["requiredwords"])
        + words_to_list(filters["requiredwords"])
        + words_to_list(category["requiredwords"])
    )
    preferred_groups = (
        words_to_list(quality["preferredwords"])
        + words_to_list(filters["preferredwords"])
        + words_to_list(category["preferredwords"])
    )
    ignored_groups = (
        words_to_list(quality["ignoredwords"])
        + words_to_list(filters["ignoredwords"])
        + words_to_list(category["ignoredwords"])
    )
    ...
    if ignored_groups and ignored_groups != [""]:
        if title:
            ignored_groups = [
                word_group
                for word_group in ignored_groups
                if not all(word in title for word in word_group)
            ]
            logging.debug(
                "ignored groups not in movie title: {}".format(ignored_groups)
            )
    ...
    if core.CONFIG["Search"]["retention"] > 0 and any(
        i["type"] == "nzb" for i in releases
    ):
    ...
    if any(i["type"] in ("torrent", "magnet") for i in releases):
        ...
        if (
            core.CONFIG["Search"]["freeleechpoints"] > 0
            or core.CONFIG["Search"]["requirefreeleech"]
        ):
            releases = freeleech(releases)
        if (
            core.CONFIG["Search"]["seederspoints"]
            and core.CONFIG["Search"]["seedersthreshold"]
        ):
            releases = threshold_score(
                releases,
                "seeders",
                core.CONFIG["Search"]["seederspoints"],
                core.CONFIG["Search"]["seedersthreshold"],
            )
        if (
            core.CONFIG["Search"]["leecherspoints"]
            and core.CONFIG["Search"]["leechersthreshold"]
        ):
            releases = threshold_score(
                releases,
                "leechers",
                core.CONFIG["Search"]["leecherspoints"],
                core.CONFIG["Search"]["leechersthreshold"],
            )
    ...
    for r in releases:
        ...
        for word_group in group_list:
            if all(word in r["title"].lower() for word in word_group):
                logging.debug(
                    "{} found in {}, removing from releases.".format(
                        word_group, r["title"]
                    )
                )
                r["reject_reason"] = "ignored words found ({})".format(
                    " ".join(word_group)
                )
                reject += 1
                break
...
def keep_required(releases, group_list):
    ...
    for r in releases:
        ...
        for word_group in group_list:
            if all(word in r["title"].lower() for word in word_group):
                logging.debug(
                    "{} found in {}, keeping this search result.".format(
                        word_group, r["title"]
                    )
                )
                required_group = True
...
def retention_check(releases):
    ...
    logging.info(
        "Checking retention [threshold = {} days].".format(
            core.CONFIG["Search"]["retention"]
        )
    )
    reject = 0
    for result in releases:
        ...
        if result["type"] == "nzb":
            ...
            if age >= core.CONFIG["Search"]["retention"]:
                logging.debug(
                    "{} published {} days ago, removing search result.".format(
                        result["title"], age
                    )
                )
                result["reject_reason"] = "older than retention ({})".format(
                    core.CONFIG["Search"]["retention"]
                )
                reject += 1
    ...
    for result in releases:
        ...
        if result["type"] in ("torrent", "magnet"):
            if int(result["seeders"]) < core.CONFIG["Search"]["mintorrentseeds"]:
                logging.debug(
                    "{} has {} seeds, removing search result.".format(
                        result["title"], result["seeders"]
                    )
                )
                result["reject_reason"] = "not enough seeds ({})".format(
                    core.CONFIG["Search"]["mintorrentseeds"]
                )
                reject += 1
    ...
    for release in releases[:]:
        ...
        if release["freeleech"] == 1:
            if core.CONFIG["Search"]["requirefreeleech"]:
                continue
            logging.debug(
                "Adding {} Freeleech points to {}.".format(points, release["title"])
            )
            release["score"] += points
        elif core.CONFIG["Search"]["requirefreeleech"]:
            logging.debug(
                "{} is not Freeleech, rejecting search result.".format(release["title"])
            )
...
def score_preferred(releases, group_list):
    ...
    for r in releases:
        for word_group in group_list:
            if all(word in r["title"].lower() for word in word_group):
                logging.debug(
                    "{} found in {}, adding 10 points.".format(word_group, r["title"])
                )
                r["score"] += 10
            else:
                continue
    ...
    if titles == [] or titles == [None]:
        ...
        for result in releases:
            ...
            if any(match > 70 for match in matches):
                ...
                logging.debug(
                    "{} best title match was {}%, removing search result.".format(
                        result["title"], max(matches)
                    )
                )
                result["reject_reason"] = "mismatch title (best match was {}%)".format(
                    max(matches)
                )
                reject += 1
...
def score_sources(releases, sources, check_size=True):
    ...
    for result in releases:
        ...
        logging.debug(
            "Scoring and filtering {} based on resolution {}.".format(
                result["title"], result_res
            )
        )
        ...
        for k, v in sources.items():
            ...
            if result_res == k:
                logging.debug(
                    "{} matches source {}, checking size.".format(result["title"], k)
                )

                if result["type"] != "import" and not (min_size < size < max_size):
                    logging.debug(
                        "Removing {}, size {} not in range {}-{}.".format(
                            result["title"], size, min_size, max_size
                        )
                    )
                    result["reject_reason"] = "size {} not in range {}-{}".format(
...
def generate_simulacrum(movie):
    ...
    logging.info(
        'Creating "fake" search result for imported movie {}'.format(movie["title"])
    )
    ...
    title = "{}.{}.{}.{}.{}.{}".format(
        ...
        or ".",  # Kind of a hacky way to make sure it doesn't print None in the title
    ...
    result["guid"] = (
        movie.get("guid")
        or "import{}".format(
            b16encode(title.encode("ascii", errors="ignore"))
            .decode("utf-8")
            .zfill(16)[:16]
        ).lower()
    )
