...
def grab_all():
    ...
    for movie in movies:
        ...
        if status == "Found":
            logging.info(
                "{} status is Found. Running automatic snatcher.".format(title)
            )
        ...
        if status == "Finished" and keepsearching is True:
            ...
            if finished_date_obj + keepsearchingdelta >= today:
                ...
                logging.info(
                    "{} {} was marked Finished on {}. Checking for a better release (min score {}).".format(
                        title, year, finished_date, minscore
                    )
                )
...
def get_best_release(movie, minscore=0, ignore_guid=None):
    ...
    if not core.CONFIG["Downloader"]["Sources"]["torrentenabled"]:
        search_results = [
            i for i in search_results if i["type"] not in ("torrent", "magnet")
        ]

    if not search_results:
        logging.warning(
            "Unable to automatically grab {}, no results available for enabled download client.".format(
                imdbid
            )
        )
        return {}
    ...
    today = datetime.datetime.today()
    release_weeks_old = (
        today - datetime.datetime.strptime(release_date, "%Y-%m-%d")
    ).days / 7
    ...
    if (today - date_found).days < wait_days:
        if (
            core.CONFIG["Search"]["skipwait"]
            and release_weeks_old > core.CONFIG["Search"]["skipwaitweeks"]
        ):
            logging.info(
                "{} released {} weeks ago, skipping wait and grabbing immediately.".format(
                    title, release_weeks_old
                )
            )
        else:
            logging.info(
                "Earliest found result for {} is {}, waiting {} days to grab best result.".format(
                    imdbid, date_found, wait_days
                )
            )
            return {}
    ...
    for result in search_results:
        ...
        if status == "Available" and result["score"] > minscore:
            logging.info(
                "{} is best available result for {}".format(result["title"], title)
            )
            return result
        # if doing a re-search, if top ranked result is Snatched we have nothing to do.
        elif status in ("Snatched", "Finished"):
            logging.info(
                "Top-scoring release for {} has already been snatched.".format(imdbid)
            )
            return {}
        else:
            continue
...
def download(data):
    ...
    if data["type"] == "nzb":
        if core.CONFIG["Downloader"]["Sources"]["usenetenabled"]:
            ...
            return {
                "response": False,
                "message": "NZB submitted but nzb snatching is disabled.",
            }
    ...
    if data["type"] in ("torrent", "magnet"):
        if core.CONFIG["Downloader"]["Sources"]["torrentenabled"]:
            ...
            return {
                "response": False,
                "message": "Torrent submitted but torrent snatching is disabled.",
            }
        ...
        if config["enabled"]:
            ...
            if response["response"]:
                ...
                core.sql.update_multiple_values(
                    "SEARCHRESULTS", db_update, "guid", guid
                )

                if update_status_snatched(guid, imdbid):
                    return {
                        "response": True,
                        "message": "Sent to {}.".format(client),
                        "download_client": client,
                        "downloadid": response["downloadid"],
                    }
                else:
                    return {
                        "response": False,
                        "error": "Could not mark search result as Snatched.",
                    }
            else:
                return response
...
def snatch_torrent(data):
    ...
    if urllib.parse.urlparse(guid).netloc:
        ...
        if guid_:
            ...
            return {
                "response": False,
                "error": "Unable to get torrent hash from indexer.",
            }
