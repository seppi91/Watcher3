...
def _login(url, account, password):
    ...
    url = "{}/webapi/auth.cgi?api=SYNO.API.Auth&version=2&method=login&account={}&passwd={}&session=DownloadStation&format=cookie".format(
...
def add_nzb(data):
    ...
    url = "{}/webapi/DownloadStation/task.cgi?api=SYNO.DownloadStation.Task&version=2&method=create&uri={}&destination={}&_sid={}".format(
        url_base, urlquote(data["guid"]), conf["destination"], cookie
    )
...
def add_torrent(data):
    ...
    url = "{}/webapi/DownloadStation/task.cgi?api=SYNO.DownloadStation.Task&version=2&method=create&uri={}&destination={}&_sid={}".format(
        url_base, urlquote(data["torrentfile"]), conf["destination"], cookie
    )
...
def get_task_id(url_base, uri):
    ...
    url = "{}/webapi/DownloadStation/task.cgi?api=SYNO.DownloadStation.Task&version=2&method=list&additional=detail&_sid={}".format(
        url_base, cookie
    )
...
def cancel_download(downloadid):
    ...
    url = "{}/webapi/DownloadStation/task.cgi?api=SYNO.DownloadStation.Task&version=1&method=delete&id={}&force_complete=false&_sid={}".format(
        url_base, task_id, cookie
    )
    ...
    if response["data"][0]["error"] != 0:
        logging.error(
            "Cannot cancel DownloadStation download: {}".format(
                errors.get(response["data"][0]["error"], errors[100])
            )
        )
        return False
