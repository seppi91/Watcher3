...
def add_torrent(data):
    ...
    try:
        ...
        logging.info(
            "Torrent sent to TransmissionRPC - downloadid {}".format(downloadid)
        )
...
def set_torrent_limits(downloadid):
    ...
    logging.info(
        "Setting idle limit to {} and ratio limit to {} for torrent #{}".format(
            idle_limit_desc, ratio_limit_desc, downloadid
        )
    )
    ...
    if args:
        ...
        try:
            ...
            logging.error(
                "Unable to change torrent #{} in TransmissionRPC.".format(downloadid),
                exc_info=True,
            )
            return False
    else:
