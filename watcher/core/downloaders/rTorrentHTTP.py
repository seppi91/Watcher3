...
def add_torrent(data):
    ...
    """
    logging.info(
        "Sending torrent {} to rTorrent HTTP RPC Plugin.".format(data["title"])
    )
    ...
    try:
        ...
        if conf["label"] and downloadid:
            ...
            while t < 10:
                ...
                t += 1
            logging.error(
                "Torrent hash ({}) not found in rTorrent after 10 seconds, cannot apply label.".format(
                    downloadid
                )
            )
            return {
                "response": False,
                "error": "Torrent hash not found in rTorrent after 10 seconds, cannot apply label.",
            }
        else:
            return {"response": True, "downloadid": downloadid}
...
def cancel_download(downloadid):
    ...
    """
    logging.info(
        "Cancelling download # {} in rTorrent HTTP RPC Plugin.".format(downloadid)
    )
