...
def _send(method, post_data=None):
    ...
    if not cookie:
        r = _login(
            "{}:{}/".format(conf["host"], conf["port"]), conf["user"], conf["pass"]
        )
        if r is not True:
    ...
    if response.status_code == 403:
        ...
        if _login(u, conf["user"], conf["pass"]) is not True:
            ...
            try:
                response = Url.open(
                    url, post_data=post_data, headers={"cookie": cookie}
                )
            except Exception as e:
                logging.error("Unable to contact QBittorrent API.", exc_info=True)
                raise APIConnectionError(response.status_code, response.reason)
...
def cancel_download(downloadid):
    ...
    try:
        _send(
            "torrents/delete",
            post_data={"hashes": downloadid.lower(), "deleteFiles": False},
        )
        return True
    except (SystemExit, KeyboardInterrupt):
        raise
