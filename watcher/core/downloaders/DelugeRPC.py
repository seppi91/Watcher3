...
def add_torrent(torrent):
    ...
    try:
        def_download_path = client.call("core.get_config")[b"download_location"].decode(
            "utf-8"
        )
    except Exception as e:
    ...
    if torrent["type"] == "magnet":
        try:
            downloadid = client.call(
                "core.add_torrent_magnet", torrent["torrentfile"], options
            ).decode("utf-8")
        except Exception as e:
        ...
        try:
            downloadid = (
                client.call("core.add_torrent_url", torrent["torrentfile"], options)
                or b""
            ).decode("utf-8")
        except Exception as e:
        ...
        return {
            "response": False,
            "error": "Invalid torrent type {}".format(torrent["type"]),
        }
...
def get_torrents_status(stalled_for=None, progress={}):
    ...
    try:
        ...
        for id, torrent in client.call(
            "core.get_torrents_status", {"id": list(progress.keys())}, fields
        ).items():
            ...
            data = {
                "hash": torrent[b"hash"].decode(),
                "status": torrent[b"state"].lower().decode(),
                "name": torrent[b"name"].decode(),
            }
            if data["status"] == "downloading" and stalled_for:
                if (
                    b"last_seen_complete" in torrent
                    and b"time_since_download" in torrent
                ):  # deluge 2.x
                    if (
                        torrent[b"last_seen_complete"] == 0
                        or now > torrent[b"last_seen_complete"] + stalled_for * 3600
                    ):
                        if (
                            torrent[b"time_since_download"] != -1
                            and torrent[b"time_since_download"] > stalled_for * 3600
                            or torrent[b"time_since_download"] == -1
                            and torrent[b"active_time"] > stalled_for * 3600
                        ):
                            data["status"] = "stalled"
