...
from watcher.core.helpers import Torrent as th
...
def test_connection(data):
    ...
    logging.info(
        "Testing ability to write to blackhole directory {}".format(data["directory"])
    )
    ...
    if core.PLATFORM == "*nix":
        ...
        try:
            ...
            logging.error(
                "Unable to write to directory {}.".format(directory), exc_info=True
            )
            return str(e)
        return True
...
def _download_magnet(data, path):
    ...
    try:
        ...
        dl_bytes = Url.open(
            ...
            post_data={"magnet": "magnet:?xt=urn:btih:{}".format(magnet_hash)},
            stream=True,
        ).content
    ...
    for i in bt_cache:
        try:
            ...
            logging.warning(
                "Unable to resolve magnet hash through {}.".format(i), exc_info=True
            )
            continue
