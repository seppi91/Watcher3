...
class API(object):
    ...
    def default(self, **params):
        ...
        logging.info(
            "API request from {}".format(cherrypy.request.headers["Remote-Addr"])
        )
    ...
    def liststatus(self, filters):
        ...
        return {
            "response": True,
            "movies": [i for i in movies if all(i[k] == v for k, v in filters.items())],
        }
    ...
    def poster(self, params):
        ...
        try:
            with open(
                os.path.abspath(
                    os.path.join(core.POSTER_DIR, "{}.jpg".format(params["imdbid"]))
                ),
                "rb",
            ) as f:
                img = f.read()
