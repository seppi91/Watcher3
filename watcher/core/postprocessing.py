...
class Postprocessing(object):
        ...
        if conf["downloadwhencomplete"]:
            ...
            if data["guid"]:
                ...
                if Manage.markedresults(
                    data["guid"], "Finished", imdbid=data["imdbid"]
                ):
            ...
            if data.get("imdbid"):
                db_update = {
                    "finished_file": "https://app.put.io/files/{}".format(
                        transfer_data["file_id"]
                    ),
                    "status": "finished",
                }
                core.sql.update_multiple_values(
                    "MOVIES", db_update, "imdbid", data["imdbid"]
                )
    ...
    def default(self, **data):
        ...
        **data: keyword params send through POST request payload
        ...
        data["original_file"] = self.get_movie_file(
            data["path"], check_size=False if data["mode"] == "failed" else True
        )
        data["parent_dir"] = (
            os.path.basename(os.path.dirname(data["original_file"]))
    ...
    def get_movie_file(self, path, check_size=True):
        ...
        if os.path.isfile(path):
            ...
            if biggestfile:
                ...
                if (
                    check_size
                    and os.path.getsize(os.path.join(path, biggestfile)) < minsize
                ):
                    logging.info(
                        "Largest file in directory {} is {}, but is smaller than the minimum size of {} bytes".format(
                            path, biggestfile, minsize
                        )
                    )
                    return None
                logging.info(
                    "Largest file in directory {} is {}, processing this file.".format(
                        path, biggestfile.replace(path, "")
                    )
                )
            else:
                logging.warning(
                    "Unable to determine largest file. Postprocessing may fail at a later point."
                )
    ...
    def get_movie_info(self, data):
        ...
        if not result:  # not found from guid or downloadid
            ...
            if fname:
                logging.info(
                    "Searching local database for release name {}".format(fname)
                )
                ...
                if result:
                    ...
                    logging.info(
                        "Unable to find local release info by release name, trying fuzzy search."
                    )
                    result = core.sql.get_single_search_result(
                        "title", re.sub(r"[\[\]\(\)\-.:]", "_", fname), like=True
                    )
                    if result:
                        logging.info("Found match for {} in releases.".format(fname))
                    else:
                        logging.info(
                            "Unable to find local release info by release name."
                        )
        ...
        # Still no luck? Try to get the info from TMDB
            ...
            logging.info(
                "Unable to find local data for release. Using only data found from file."
            )
    ...
    def complete(self, data):
        ...
        if data.get("imdbid"):
            if core.sql.row_exists("MOVIES", imdbid=data["imdbid"]):
                data["category"] = core.sql.get_movie_details("imdbid", data["imdbid"])[
                    "category"
                ]
            else:
                logging.info(
                    "{} not found in library, adding now.".format(data.get("title"))
                )
        ...
        if config["cleanupenabled"]:
            ...
            if (
                config["moverenabled"] is False
                or result["tasks"]["mover"]["response"] is False
            ):
    ...
    def renamer(self, data):
        ...
        logging.info(
            "Renaming {} to {}".format(
                os.path.basename(data.get("original_file")), new_name
            )
        )
        try:
    ...
    def recycle(self, recycle_bin, abs_filepath):
        ...
        if not os.path.isdir(recycle_bin):
            ...
            try:
                ...
                logging.error(
                    "Recycling failed: Could not create Recycle Bin directory {}.".format(
                        recycle_bin
                    ),
                    exc_info=True,
                )
    ...
    def mover(self, data):
        ...
        try:
            ...
            logging.error(
                "Mover failed: Could not create directory {}.".format(target_folder),
                exc_info=True,
            )
            return ""
            ...
            if os.path.isfile(old_movie):
                ...
                if config["recyclebinenabled"]:
                    ...
                    try:
                        ...
                        logging.error(
                            "Mover failed: Could not delete file.", exc_info=True
                        )
                        return ""
        ...
        if os.path.isfile(os.path.join(target_folder, file_name)):
            ...
            logging.info(
                "Existing file {} found in {}".format(file_name, target_folder)
            )
            if config["recyclebinenabled"]:
        ...
        new_file_location = os.path.join(
            target_folder, os.path.basename(data["original_file"])
        )

        if config["movermethod"] == "hardlink":
            logging.info(
                "Creating hardlink from {} to {}.".format(
                    data["original_file"], new_file_location
                )
            )
            try:
            ...
            if core.PLATFORM == "windows":
                logging.warning(
                    "Attempting to create symbolic link on Windows. This will fail without SeCreateSymbolicLinkPrivilege."
                )
            logging.info(
            ...
            try:
                ...
                logging.error(
                    "Mover failed: Unable to create symbolic link.", exc_info=True
                )
                return ""
        elif config["movermethod"] == "copy":
            logging.info(
                "Copying {} to {}.".format(data["original_file"], new_file_location)
            )
            try:
            ...
            if config["movermethod"] == "symboliclink":
                ...
                logging.info(
                    "Creating symbolic link from {} to {}".format(
                        new_file_location, data["original_file"]
                    )
                )
                try:
        ...
        keep_extensions = [
            i.strip() for i in config["moveextensions"].split(",") if i != ""
        ]

        if len(keep_extensions) > 0:
            logging.info(
                "Moving additional files with extensions {}.".format(
                    ",".join(keep_extensions)
                )
            )
            ...
            for root, dirs, filenames in os.walk(data["path"]):
                for name in filenames:
                    ...
                    target_file = "{}{}".format(
                        os.path.join(target_folder, fname), target_ext
                    )
                    ...
                    if ext.replace(".", "") in keep_extensions:
                        ...
                        while os.path.isfile(target_file):
                            ...
                            target_file = "{}{}".format(
                                os.path.join(target_folder, new_filename), target_ext
                            )
                        try:
                            logging.info(
                                "Moving {} to {}".format(old_abs_path, target_file)
                            )
                            shutil.copyfile(old_abs_path, target_file)
                        except Exception as e:  # noqa
                            logging.error(
                                "Moving additional files failed: Could not copy {}.".format(
                                    old_abs_path
                                ),
                                exc_info=True,
                            )
