...
def create():
    ...
    if core.CONFIG["Server"]["Proxy"]["type"] == "socks5":
        ...
        if protocol == "https":
            ...
            logging.warning(
                "HTTP-only proxy. HTTPS traffic will not be tunneled through proxy."
            )
        ...
        logging.warning(
            "Invalid proxy type {}".format(core.CONFIG["Server"]["Proxy"]["type"])
        )
        return
...
def whitelist(url):
    ...
    for i in whitelist:
        if url.startswith(i.strip()):
            logging.info(
                "{} in proxy whitelist, will bypass proxy connection.".format(url)
            )
            return True
        else:
            continue
