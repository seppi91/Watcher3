...
def sync():
    ...
    for url in core.CONFIG["Search"]["Watchlists"]["imdbcsv"]:
        ...
        last_sync = datetime.strptime(
            (record.get(list_id) or "2000-01-01"), date_format
        )
    ...
    for imdbid in movies_to_add:
        ...
        if not movie:
            ...
            movie = movie[0]
        logging.info(
            "Adding movie {} {} from IMDB watchlist.".format(
                movie["title"], movie["imdbid"]
            )
        )
        movie["year"] = (
            movie["release_date"][:4] if movie.get("release_date") else "N/A"
        )
        movie["origin"] = "IMDB"
    ...
    if core.sql.row_exists("SYSTEM", name="imdb_sync_record"):
        core.sql.update(
            "SYSTEM", "data", json.dumps(record), "name", "imdb_sync_record"
        )
    else:
        core.sql.write(
            "SYSTEM", {"data": json.dumps(record), "name": "imdb_sync_record"}
