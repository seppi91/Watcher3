...
def check_all():
    ...
    backlog_movies = [
        i
        for i in movies
        if i["predb_backlog"] != "1" and i["status"] not in ("Disabled", "Finished")
    ]
    rss_movies = [
        i
        for i in movies
        if i["predb_backlog"] == "1"
        and i["predb"] != "found"
    ...
    if backlog_movies:
        logging.info(
            "Performing predb backlog search for {}".format(
                ", ".join(i["title"] for i in backlog_movies)
            )
        )
...
def _search_rss(movies):
    ...
    logging.info(
        "Checking predb rss for {}".format(", ".join(i["title"] for i in movies))
    )
