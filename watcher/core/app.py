...
class App(object):
    ...
    status_template = Template(
        filename="templates/library/status.html", module_directory=core.MAKO_CACHE
    )
    manage_template = Template(
        filename="templates/library/manage.html", module_directory=core.MAKO_CACHE
    )
    import_template = Template(
        filename="templates/library/import.html", module_directory=core.MAKO_CACHE
    )
    couchpotato_template = Template(
        filename="templates/library/import/couchpotato.html",
        module_directory=core.MAKO_CACHE,
    )
    kodi_template = Template(
        filename="templates/library/import/kodi.html", module_directory=core.MAKO_CACHE
    )
    plex_template = Template(
        filename="templates/library/import/plex.html", module_directory=core.MAKO_CACHE
    )
    ...
    stats_template = Template(
        filename="templates/library/stats.html", module_directory=core.MAKO_CACHE
    )

    add_movie_template = Template(
        filename="templates/add_movie.html", module_directory=core.MAKO_CACHE
    )

    server_template = Template(
        filename="templates/settings/server.html", module_directory=core.MAKO_CACHE
    )
    search_template = Template(
        filename="templates/settings/search.html", module_directory=core.MAKO_CACHE
    )
    quality_template = Template(
        filename="templates/settings/quality.html", module_directory=core.MAKO_CACHE
    )
    categories_template = Template(
        filename="templates/settings/categories.html", module_directory=core.MAKO_CACHE
    )
    indexers_template = Template(
        filename="templates/settings/indexers.html", module_directory=core.MAKO_CACHE
    )
    downloader_template = Template(
        filename="templates/settings/downloader.html", module_directory=core.MAKO_CACHE
    )
    ...
    plugins_template = Template(
        filename="templates/settings/plugins.html", module_directory=core.MAKO_CACHE
    )
    logs_template = Template(
        filename="templates/settings/logs.html", module_directory=core.MAKO_CACHE
    )
    system_template = Template(
        filename="templates/settings/system.html", module_directory=core.MAKO_CACHE
    )

    shutdown_template = Template(
        filename="templates/system/shutdown.html", module_directory=core.MAKO_CACHE
    )
    restart_template = Template(
        filename="templates/system/restart.html", module_directory=core.MAKO_CACHE
    )
    update_template = Template(
        filename="templates/system/update.html", module_directory=core.MAKO_CACHE
    )

    fourohfour_template = Template(
        filename="templates/404.html", module_directory=core.MAKO_CACHE
    )
    head_template = Template(
        filename="templates/head.html", module_directory=core.MAKO_CACHE
    )
    navbar_template = Template(
        filename="templates/navbar.html", module_directory=core.MAKO_CACHE
    )
    ...
    def library(self, *path):
        ...
        if page == "status":
            ...
            status_count["Finished"] = status_count.get(
                "Finished", 0
            ) + status_count.get("Disabled", 0)
            ...
            return App.status_template.render(
                ...
                categories=core.CONFIG["Categories"].keys(),
            ...
            App.stats_template = Template(
                filename="templates/library/stats.html",
                module_directory=core.MAKO_CACHE,
            )

            return App.stats_template.render(
    ...
    @cherrypy.expose
    def add_movie(self):
        return App.add_movie_template.render(
            profiles=[
                (k, v.get("default", False))
                for k, v in core.CONFIG["Quality"]["Profiles"].items()
            ],
            categories=core.CONFIG["Categories"].keys(),
            **self.defaults()
        )
    ...
    def settings(self, *path):
        ...
        if page == "server":
            themes = [
                i[:-4]
                for i in os.listdir("static/css/themes/")
                if i.endswith(".css")
                and os.path.isfile(os.path.join(core.PROG_PATH, "static/css/themes", i))
            ]
            ...
            return App.search_template.render(
                config=core.CONFIG["Search"], **self.defaults()
            )
        elif page == "quality":
            return App.quality_template.render(
                config=core.CONFIG["Quality"], sources=core.SOURCES, **self.defaults()
            )
        elif page == "categories":
            return App.categories_template.render(
                config=core.CONFIG["Categories"],
                sources=core.SOURCES,
                **self.defaults()
            )
        elif page == "indexers":
            ...
            return App.indexers_template.render(
                config=core.CONFIG["Indexers"], **self.defaults()
            )
            ...
            return App.plugins_template.render(
                config=core.CONFIG["Plugins"], plugins=plugs, **self.defaults()
            )
        elif page == "logs":
            logdir = os.path.join(core.PROG_PATH, core.LOG_DIR)
            logfiles = [
                i for i in os.listdir(logdir) if os.path.isfile(os.path.join(logdir, i))
            ]
            return App.logs_template.render(
                logdir=logdir, logfiles=logfiles, **self.defaults()
            )
            ...
            return App.system_template.render(
                config=core.CONFIG["System"],
    ...
    def head(self):
        return App.head_template.render(
            ...
            notifications=json.dumps([i for i in core.NOTIFICATIONS if i is not None]),
            language=core.LANGUAGE,
        )

    def nav_bar(self, current=None):
        username = cherrypy.session.get(core.SESSION_KEY)
        return App.navbar_template.render(
            url_base=core.URL_BASE, current=current, username=username
        )
