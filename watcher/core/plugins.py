    ...
    for folder in plugins.keys():
        try:
            ...
            a = [
                ...
                if i.endswith(".py") and not i.startswith(".")
            ]
            ...
            logging.error(
                "Unable to read {} plugins folder".format(folder), exc_info=True
            )
...
def _config_string(config):
    ...
    base = {"label": "&nbsp", "helptext": "", "value": ""}
...
def _config_int(config):
    ...
    base = {"label": "&nbsp", "helptext": "", "value": "", "min": "", "max": ""}
...
def _config_bool(config):
    ...
    base = {"label": "&nbsp", "helptext": "", "value": False}
...
def added(*args):
    ...
    *args (list): arguments to pass to plugins
    ...
    sorted_plugins = sorted(
        core.CONFIG["Plugins"]["added"].items(), key=operator.itemgetter(1)
    )
...
def snatched(*args):
    ...
    *args (list): arguments to pass to plugins
    ...
    sorted_plugins = sorted(
        core.CONFIG["Plugins"]["snatched"].items(), key=operator.itemgetter(1)
    )
...
def finished(*args):
    ...
    *args (list): arguments to pass to plugins
    ...
    sorted_plugins = sorted(
        core.CONFIG["Plugins"]["finished"].items(), key=operator.itemgetter(1)
    )
    ...
    for plugin in plugins:
        ...
        try:
            ...
            if exit_code == 0:
                ...
                logging.warning(
                    "{} - Execution failed. Exit code {}.".format(name, exit_code)
                )
