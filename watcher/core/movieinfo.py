...
class TheMovieDatabase(object):
    ...
    def _search_title(title):
        ...
        url = "https://api.themoviedb.org/3/search/movie?page=1&include_adult={}&".format(
    ...
    def _search_imdbid(imdbid):
        ...
        url = "https://api.themoviedb.org/3/find/{}?language=en-US&external_source=imdb_id&append_to_response=alternative_titles,external_ids,release_dates".format(
            imdbid
        )
    ...
    def _search_tmdbid(tmdbid):
        ...
        url = "https://api.themoviedb.org/3/movie/{}?language=en-US&append_to_response=alternative_titles,external_ids,release_dates".format(
            tmdbid
        )
        ...
        try:
            ...
            if response.status_code != 200:
                logging.warning(
                    "Unable to reach TMDB, error {}".format(response.status_code)
                )
                return []
            else:
    ...
    def get_imdbid(tmdbid=None, title=None, year=""):
        ...
        if not tmdbid:
            ...
            try:
                ...
                logging.error(
                    "Error attempting to get TMDBID from TMDB.", exc_info=True
                )
                return ""
    ...
    def get_category(cat, tmdbid=None):
        ...
        if cat == "similar":
            if tmdbid is None:
                return []
            url = "https://api.themoviedb.org/3/movie/{}/similar?&language=en-US&page=1".format(
                tmdbid
            )
        else:
            url = "https://api.themoviedb.org/3/movie/{}?language=en-US&page=1".format(
                cat
            )
        ...
        try:
            ...
            logging.error(
                "Unable to read {} movies from TheMovieDB.".format(cat), exc_info=True
            )
            return []
...
class YouTube(object):
    ...
    def trailer(title_date):
        ...
        url = "https://www.googleapis.com/youtube/v3/search?part=snippet&q={}&maxResults=1&key={}".format(
