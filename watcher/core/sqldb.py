...
class SQL(object):
    ...
    def __init__(self):
        ...
        try:
            self.engine = sqla.create_engine(
                DB_NAME, echo=False, connect_args={"timeout": 30}
            )
        ...
        while tries < 5:
            try:
                ...
                if "database is locked" in e.args[0]:
                    ...
                    logging.error(
                        "SQL Databse Query: {}.".format(command), exc_info=True
                    )
                    return None
        # all tries exhausted
        return None
    ...
    def update(self, TABLE, COLUMN, VALUE, idcol, idval):
        ...
        logging.debug(
            "Updating {} to {} for rows that match {}:{} in {}.".format(
                COLUMN, VALUE, idcol, idval.split("&")[0], TABLE
            )
        )

        sql = "UPDATE {} SET {}=? WHERE {}=? COLLATE NOCASE".format(
            TABLE, COLUMN, idcol
        )
        vals = (VALUE, idval)
    ...
    def update_multiple_values(self, TABLE, data, idcol, idval):
        ...
        logging.debug(
            "Updating {}:{} to {} in {}.".format(
                idcol, idval.split("&")[0], data, TABLE
            )
        )
    ...
    def update_multiple_rows(self, TABLE, values, id_col):
        ...
        core.sql.engine.execute(
            TABLE.update()
            .where(getattr(TABLE.c, id_col) == sqla.bindparam(id_col_))
            .values(write),
            values,
        )
    ...
    def get_user_movies(
        ...
        command = "SELECT * FROM MOVIES {} ORDER BY {} {}".format(
            filters, sort_key, sort_direction
        )
    ...
    def get_library_count(self, group_col=None, col=None, val=None):
        ...
        if group_col:
            query = "SELECT {}, COUNT(1) FROM MOVIES {} GROUP BY {}".format(
                group_col, filters, group_col
            )
        else:
    ...
    def get_movies_status(self, idcol, idvals):
        ...
        command = [
            "SELECT {}, status FROM MOVIES WHERE {} IN ({})".format(
                idcol, idcol, ", ".join(map(str, idvals))
            )
        ]
    ...
    def get_search_results(self, imdbid, quality=None, rejected=False):
        ...
        if (
            quality in core.CONFIG["Quality"]["Profiles"]
            and core.CONFIG["Quality"]["Profiles"][quality]["prefersmaller"]
        ):
            sort = "ASC"
        else:
            sort = "DESC"
        ...
        if core.CONFIG["Search"]["preferredsource"] == "":
            ...
            sk = """, CASE type WHEN "nzb" THEN 0
                ...
                """.format(
                "ASC"
                if core.CONFIG["Search"]["preferredsource"] == "usenet"
                else "DESC"
            )
        ...
        command = [
            'SELECT * FROM SEARCHRESULTS WHERE imdbid="{}" {} ORDER BY score DESC {}, size {}, freeleech DESC'.format(
                imdbid, rejected_cond, sk, sort
            )
        ]
    ...
    def delete(self, TABLE, idcol, idval):
        ...
        logging.debug(
            "Removing from {} where {} is {}.".format(TABLE, idcol, idval.split("&")[0])
        )
    ...
    def get_distinct(self, TABLE, column, idcol, idval):
        ...
        logging.debug(
            "Getting distinct values for {} in {}".format(idval.split("&")[0], TABLE)
        )

        command = [
            'SELECT DISTINCT {} FROM {} WHERE {}="{}"'.format(
                column, TABLE, idcol, idval
            )
        ]
    ...
    def row_exists(self, TABLE, **cols):
        ...
        logging.debug(
            "Checking if {} exists in database table {}".format(
                ",".join(matches), TABLE
            )
        )

        command = [
            "SELECT 1 FROM {} WHERE {} COLLATE NOCASE".format(
                TABLE, " AND ".join(matches)
            )
        ]
    ...
    def get_download_progress(self, client):
        ...
        if data:
            for i in data.fetchall():
                results[i["downloadid"]] = {
                    "progress": i["download_progress"],
    ...
    def get_single_search_result(self, idcol, idval, like=False):
        ...
        logging.debug(
            "Retrieving search result details for {}.".format(idval.split("&")[0])
        )

        operator = "LIKE" if like else "="
        command = [
            'SELECT * FROM SEARCHRESULTS WHERE {} {} "{}" COLLATE NOCASE ORDER BY score DESC, size DESC'.format(
                idcol, operator, idval
            )
        ]
            ...
            for name, kind in schema.items():
                ...
                if table in SQL.convert_names.keys():
                    for pair in SQL.convert_names[table]:
                        if pair[0] == name:
                            command = [
                                "UPDATE {} SET {} = {}".format(table, pair[0], pair[1])
                            ]
                            self.execute(command)
