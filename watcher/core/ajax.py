...
class Ajax(object):
    ...
    def library(
        self, sort_key, sort_direction, limit=50, offset=0, status=None, category=None
    ):
        ...
        return core.sql.get_user_movies(
            sort_key, sort_direction.upper(), limit, offset, status, category
        )
    ...
    def library_counters(self, category=None):
        ...
        status_count["Finished"] = status_count.get("Finished", 0) + status_count.get(
            "Disabled", 0
        )
    ...
    def add_wanted_movie(self, data):
        ...
        if (
            response["response"]
            and core.CONFIG["Search"]["searchafteradd"]
            and movie["year"] != "N/A"
        ):
    ...
    def delete_movie_file(self, imdbid):
        ...
        try:
            ...
            core.sql.update_multiple_values(
                "MOVIES",
                {"finished_date": None, "finished_score": None, "finished_file": None},
                "imdbid",
                imdbid,
            )
    ...
    def search(self, imdbid):
        ...
        if not movie:
            ...
            success = searcher.search(
                imdbid, movie["title"], movie["year"], movie["quality"]
            )
            status = core.sql.get_movie_details("imdbid", imdbid)["status"]

            if success:
                results = core.sql.get_search_results(
                    imdbid, movie["quality"], rejected=True
                )
                ...
                return {
                    "response": False,
                    "error": Errors.database_read.format(imdbid),
                    "movie_status": status,
                }
    ...
    def manual_download(self, year, guid, kind):
        ...
        if kind == "nzb" and not usenet_enabled:
            return {
                "response": False,
                "error": _("Link is NZB but no Usent client is enabled."),
            }
        elif kind in ("torrent", "magnet") and not torrent_enabled:
            return {
                "response": False,
                "error": _("Link is Torrent/Magnet but no Torrent client is enabled."),
            }
    ...
    def mark_bad(self, guid, imdbid, cancel_download=False):
        ...
        if cancel_download:
            ...
            if not cancelled:
                response["response"] = False
                response["error"] = response.get("error", "") + _(
                    " Could not remove download from client."
                )
    ...
    def unmark_bad(self, guid, imdbid):
        ...
        if not core.sql.delete("MARKEDRESULTS", "guid", guid):
            ...
            logging.info(
                "Successfully removed {} from MARKEDRESULTS.".format(guid.split("&")[0])
            )
    ...
    def update_server(self, mode):
        ...
        if mode == "update_now":
            ...
            active_tasks = [
                k for k, v in core.scheduler_plugin.task_list.items() if v.running
            ]
            ...
            if update_status is False:
                ...
                yield json.dumps(
                    {"response": False, "error": _("Unable to complete update.")}
                )
    ...
    def update_movie_options(self, quality, category, status, filters, imdbid):
        ...
        if not core.sql.update_multiple_values(
            "MOVIES",
            {"quality": quality, "category": category, "filters": filters},
            "imdbid",
            imdbid,
        ):
            return {"response": False, "error": Errors.database_write}
        ...
        for index, path in enumerate(files):
            ...
            try:
                ...
                if not metadata.get("imdbid"):
                    ...
                    logging.info(
                        "{} ({}) already in library, ignoring.".format(
                            metadata["title"], path
                        )
                    )
                    ...
                    logging.info(
                        "Resolution/Source unknown for import {}".format(
                            metadata["title"]
                        )
                    )
                    response["response"] = "incomplete"
                    ...
                    logging.info(
                        "All data found for import {}".format(metadata["title"])
                    )
    ...
    scan_library_directory._cp_config = {
    ...
    def import_dir(self, movies, corrected_movies):
        ...
        if corrected_movies:
            logging.info(
                "{} corrected movies, gathering metadata.".format(len(corrected_movies))
            )
                    ...
                    yield json.dumps(
                        {
                            ...
                            "error": Errors.tmdb_not_found.format(data["tmdbid"]),
                        }
                    )
                    progress += 1

        logging.info(
            "Adding {} directory scan movies to library.".format(len(movie_data))
        )
                ...
                yield json.dumps(
                    {
                        ...
                        "movie": movie,
                        "progress": [progress, length],
                        "error": Errors.tmdb_not_found.format(data["title"]),
                    }
                )
                progress += 1
        ...
        for i in success:
            for r in fake_results:
                if r["imdbid"] == i["imdbid"]:
                    core.sql.update(
                        "MOVIES", "finished_score", r["score"], "imdbid", i["imdbid"]
                    )
                    break
    ...
    def list_files(self, current_dir, move_dir):
        ...
        try:
            response["list"] = [
                ...
                if os.path.isdir(os.path.join(new_path, i)) and not i.startswith(".")
            ]
        except Exception as e:
                ...
                yield json.dumps(
                    {
                        ...
                        "movie": movie,
                        "progress": [progress, length],
                        "error": Errors.tmdb_not_found.format("NONE"),
                    }
                )
                progress += 1
                continue
                ...
                yield json.dumps(
                    {
                        ...
                        "movie": movie,
                        "progress": [progress, length],
                        "error": Errors.tmdb_not_found.format(movie["imdbid"]),
                    }
                )
                progress += 1
                continue
    ...
    def import_plex_csv(self, movies, corrected_movies):
        ...
        if corrected_movies:
            logging.info(
                "Adding {} Plex movies to library.".format(len(corrected_movies))
            )
                    ...
                    yield json.dumps(
                        {
                            ...
                            "progress": [progress, length],
                            "title": movie["title"],
                            "error": Errors.tmdb_not_found.format(movie["tmdbid"]),
                        }
                    )
                    progress += 1
                    continue
                ...
                yield json.dumps(
                    {
                        ...
                        "progress": [progress, length],
                        "error": _(
                            "Unable to find IMDB ID for {} on TheMovieDB."
                        ).format(movie["title"]),
                        "title": movie["title"],
                    }
                )
    ...
    def import_cp_movies(self, wanted, finished):
        ...
        logging.info(
            "Adding {} Wanted CouchPotato movies to library.".format(len(wanted))
        )
        for movie in wanted:
            response = Manage.add_movie(movie, full_metadata=True)
            if response["response"] is True:
                yield json.dumps(
                    {"response": True, "progress": [progress, length], "movie": movie}
                )
                progress += 1
                continue
            else:
        ...
        logging.info(
            "Adding {} Wanted CouchPotato movies to library.".format(len(finished))
        )
    ...
    def manager_backlog_search(self, movies):
        ...
        logging.info(
            "Performing bulk backlog search for {} movies.".format(len(movies))
        )
    ...
    manager_backlog_search._cp_config = {
        "response.stream": True,
        "tools.gzip.on": False,
    }
    ...
    def manager_update_metadata(self, movies):
        ...
        logging.info(
            "Performing bulk metadata update for {} movies.".format(len(movies))
        )
    ...
    manager_update_metadata._cp_config = {
        "response.stream": True,
        "tools.gzip.on": False,
    }
    ...
    def manager_change_quality(self, movies, quality):
        ...
        logging.info(
            "Setting quality to {} for: {}".format(
                quality, ", ".join(i["imdbid"] for i in movies)
            )
        )

        for i, movie in enumerate(movies):
            if not core.sql.update(
                "MOVIES", "quality", quality, "imdbid", movie["imdbid"]
            ):
                response = {
                    "response": False,
                    "error": Errors.database_write,
    ...
    manager_change_quality._cp_config = {
        "response.stream": True,
        "tools.gzip.on": False,
    }
    ...
    def manager_change_category(self, movies, category):
        ...
        logging.info(
            "Setting category to {} for: {}".format(
                category, ", ".join(i["imdbid"] for i in movies)
            )
        )

        for i, movie in enumerate(movies):
            if not core.sql.update(
                "MOVIES", "category", category, "imdbid", movie["imdbid"]
            ):
                response = {
                    "response": False,
                    "error": Errors.database_write,
    ...
    manager_change_category._cp_config = {
        "response.stream": True,
        "tools.gzip.on": False,
    }
        ...
        for i, movie in enumerate(movies):
            ...
            if not core.sql.purge_search_results(imdbid):
                yield json.dumps(
                    {
                        "response": False,
                        "error": _("Unable to purge search results."),
                        "imdbid": imdbid,
                        "index": i + 1,
                    }
                )
            ...
            db_reset = {
                "quality": config.default_profile(),
                "status": "Waiting",
                "finished_date": None,
                "finished_score": None,
                "backlog": 0,
    ...
    def create_backup(self):
        ...
        logging.info(
            "Creating backup of Watcher as {}".format(
                os.path.join(core.PROG_PATH, "watcher.zip")
            )
        )
        ...
        return {
            "response": True,
            "message": _("Backup created as {}").format(
                os.path.join(core.PROG_PATH, "watcher.zip")
            ),
        }
    ...
    def manual_task_execute(self, name):
        ...
        try:
            ...
            return {
                "response": True,
                "message": _("Finished task {}.").format(name),
                "last_execution": le,
