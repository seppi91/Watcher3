...
class ImportDirectory(object):
    ...
    def scan_dir(directory, minsize=500, recursive=True):
        ...
        f = []
        logging.debug(
            "Specified minimum file size: {} Bytes.".format(minsize * 1024 ** 2)
        )
        ms = minsize * 1024 ** 2
        for i in files:
            s = os.path.getsize(i)
    ...
    def get_movies(url):
        ...
        krequest = {
            ...
            "params": {
                ...
                "properties": ["title", "year", "imdbnumber", "file", "streamdetails"],
                "sort": {
                    "order": "ascending",
                    "method": "label",
                    "ignorearticle": True,
        ...
        if response.status_code != 200:
            return {
                "response": False,
                "error": "{} {}".format(response.status_code, response.reason.title()),
            }
        ...
        for i in json.loads(response.text)["result"]["movies"]:
            ...
            movie["audiocodec"] = (
                i["streamdetails"]["audio"][0].get("codec")
                if i["streamdetails"]["audio"]
                else ""
            )
    ...
    def read_csv(csv_text):
        ...
        for movie in movies:
            ...
            try:
                ...
                if parsed["audiocodec"] == "dca" or parsed["audiocodec"].startswith(
                    "dts"
                ):
...
class ImportCPLibrary(object):
    ...
    def get_movies(url):
        ...
        try:
            ...
            return {
                "response": False,
                "error": _("Malformed json response from CouchPotato"),
            }
        ...
        for m in cp["movies"]:
            if m["info"]["imdb"] in library:
                logging.debug(
                    "{} in library, skipping.".format(m["info"]["original_title"])
                )
                continue

            logging.debug(
                "Parsing CouchPotato movie {}".format(m["info"]["original_title"])
            )
            movie = {}
            if m["status"] == "done":
                ...
                for i in m["releases"]:
                    if i["status"] == "done":
                        ...
                        try:
                            movie["size"] = (
                                i.get("info", {}).get("size", 0) * 1024 * 1024
                            )
                        except Exception as e:
                            movie["size"] = 0
                        if any(i in name for i in ["4K", "UHD", "2160P"]):
                            ...
                            i in name for i in ["1080", "brrip", "bdrip", "bluray"]
                        ):
                            movie["resolution"] = "BluRay-1080P"
                        elif "720" in name:
                            movie["resolution"] = "BluRay-720P"
            ...
            movie["url"] = "https://www.themoviedb.org/movie/{}".format(
                cpm.get("tmdb_id", "")
            )
            ...
            movie["release_date"] = (
                datetime.datetime.fromtimestamp(ts).strftime("%Y-%m-%d") if ts else None
            )

            movie["alternative_titles"] = {
                "titles": [
                    {"iso_3166_1": "US", "title": ",".join(cpm.get("titles", []))}
                ]
            }
...
class Metadata(object):
    ...
    def from_file(filepath, imdbid=None):
        ...
        if data.get("title") and not data.get("imdbid"):
            ...
            if not tmdbdata:
                logging.warning(
                    "Unable to get data from TheMovieDB for {}".format(data["title"])
                )
                return data
            ...
            if not tmdbid:
                logging.warning(
                    "Unable to get data from TheMovieDB for {}".format(data["imdbid"])
                )
                return data
            ...
            if tmdbdata:
                ...
                logging.warning(
                    "Unable to get data from TMDB for {}".format(data["imdbid"])
                )
                return data
    ...
    def parse_media(filepath):
        ...
        if filedata:
            ...
            video = (
                filedata.get("Metadata")
                or filedata.get("video[1]")
                or filedata.get("video")
                or {}
            )
            ...
            if video.get("compression"):
                metadata["videocodec"] = (
                    video["compression"].split("/")[0].split("(")[0].replace("V_", "")
                )
    ...
    def parse_filename(filepath):
        ...
        logging.info(
            "Parsing directory name for movie information: {}.".format(dirname)
        )
    ...
    def convert_to_db(movie):
        ...
        logging.info(
            "Converting movie metadata to database structure for {}.".format(
                movie["title"]
            )
        )
        ...
        movie["plot"] = (
            movie.get("overview") if not movie.get("plot") else movie.get("plot")
        )
        movie["url"] = "https://www.themoviedb.org/movie/{}".format(
            movie.get("id", movie.get("tmdbid"))
        )
        ...
        if movie.get("alternative_titles") and not isinstance(
            movie.get("alternative_titles"), str
        ):
            a_t = []
        ...
        movie = {
            k: v for k, v in movie.items() if k in [i.name for i in core.sql.MOVIES.c]
        }
    ...
    def update(imdbid, tmdbid=None, force_poster=True):
        ...
        if tmdbid is None:
            ...
            if not tmdbid:
                logging.debug(
                    "TMDB id not found in local database, searching TMDB for {}".format(
                        imdbid
                    )
                )
...
class Manage(object):
    ...
    def scanmissingfiles():
        movies = core.sql.execute(
            ["SELECT * FROM MOVIES WHERE finished_file is not null"]
        )
        ...
        for i in movies:
            if not os.path.exists(i["finished_file"]):
                logging.info(
                    "File {} is missing for movie {}".format(
                        i["finished_file"], i["title"]
                    )
                )
                ...
                if action == "remove":
                    if Manage.remove_movie(i["imdbid"])["response"] is False:
                        logging.error(
                            "Unable to remove {} from library".format(i["title"])
                        )
                elif action == "revert":
                    db_update_values.append(
                        {
                            "imdbid": i["imdbid"],
                            "finished_file": None,
                            "finished_date": None,
                    ...
                    if (
                        core.sql.delete("MARKEDRESULTS", "imdbid", i["imdbid"]) is None
                        or core.sql.delete("SEARCHRESULTS", "imdbid", i["imdbid"])
                        is None
                    ):
                        logging.warning(
                            "Unable to remove marked releases. Status change may be incorrect."
                        )
    ...
    def verify(movie, today=None):
        ...
        if core.CONFIG["Search"]["verifyreleases"] == "":
            ...
            logging.info(
                "{} does not have a theatrical release date, skipping verification check as Unverified.".format(
                    movie["title"]
                )
            )
            verified = False
        elif (
            core.CONFIG["Search"]["verifyreleasesskip"]
            and datetime.datetime.strptime(movie["release_date"], "%Y-%m-%d")
            + datetime.timedelta(
                days=7 * core.CONFIG["Search"]["verifyreleasesskipweeks"]
            )
            < today
        ):
            logging.info(
                "{} is older than {}, skipping verification check as Verified.".format(
                    movie["title"], core.CONFIG["Search"]["verifyreleasesskipweeks"]
                )
            )
            verified = True
            ...
            if not movie.get("predb") and movie.get("predb_backlog"):
                logging.debug(
                    "Resetting predb backlog status for unfound movie {} {}".format(
                        movie["title"], movie["year"]
                    )
                )
                core.sql.update(
                    "MOVIES", "predb_backlog", None, "imdbid", movie["imdbid"]
                )
            if not movie.get("media_release_date"):
                logging.info(
                    "{} does not yet have a home media release date.".format(
                        movie["title"]
                    )
                )
                verified = False
            else:
                media_release = datetime.datetime.strptime(
                    movie["media_release_date"], "%Y-%m-%d"
                )
        ...
        if verified and movie["status"] == "Waiting":
            logging.info(
                "Verification criteria met for {} {}, setting status to Wanted".format(
                    movie["title"], movie["year"]
                )
            )
            core.sql.update("MOVIES", "status", "Wanted", "imdbid", movie["imdbid"])
        elif not verified and movie["status"] not in (
            "Waiting",
            "Disabled",
            "Finished",
        ):
            logging.info(
                "Verified criteria not met for {} {}, resetting setting status to Waiting".format(
                    movie["title"], movie["year"]
                )
            )
        ...
        if verified:
            logging.info(
                "{} passes verification checks, will include title in search.".format(
                    movie["title"]
                )
            )
        else:
            logging.info(
                "{} does not pass verification checks, will ignore for now.".format(
                    movie["title"]
                )
            )
    ...
    def add_movie(movie, full_metadata=False):
        ...
        if not full_metadata:
            logging.debug(
                "More information needed, searching TheMovieDB for {}".format(tmdbid)
            )
        ...
        if not core.sql.write("MOVIES", movie):
            ...
            if poster_path:
                ...
                threading.Thread(
                    target=Poster.save, args=(movie["imdbid"], poster_url)
                ).start()
            ...
            plugins.added(
                movie["title"], movie["year"], movie["imdbid"], movie["quality"]
            )
    ...
    def remove_movie(imdbid):
        ...
        if removed is True:
            response = {
                "response": True,
                "message": _("{} removed from library.").format(m.get("title")),
            }
            threading.Thread(target=Poster.remove, args=(imdbid,)).start()
        elif removed is False:
            response = {
                "response": False,
                "error": _("Unable to remove {}.").format(m.get("title")),
            }
        elif removed is None:
            response = {
                "response": False,
                "error": _("{} does not exist in library.").format(imdbid),
            }
    ...
    def searchresults(guid, status, movie_info=None):
        ...
        if core.sql.row_exists(TABLE, guid=guid):
            ...
            logging.info(
                "Marking {} as {} in SEARCHRESULTS.".format(guid.split("&")[0], status)
            )
            if not core.sql.update(TABLE, "status", status, "guid", guid):
                logging.error(
                    "Setting SEARCHRESULTS status of {} to {} failed.".format(
                        guid.split("&")[0], status
                    )
                )
                return False
                ...
                logging.info(
                    "Successfully marked {} as {} in SEARCHRESULTS.".format(
                        guid.split("&")[0], status
                    )
                )
                return True
            ...
            logging.info(
                "Guid {} not found in SEARCHRESULTS, attempting to create entry.".format(
                    guid.split("&")[0]
                )
            )
            if movie_info is None:
                logging.warning(
                    "Movie metadata not supplied, unable to create SEARCHRESULTS entry."
                )
                return False
            ...
            search_result["size"] = os.path.getsize(
                movie_info.get("orig_filename") or "."
            )
            ...
            search_result = {
                k: v for k, v in search_result.items() if k in required_keys
            }
    ...
    def markedresults(guid, status, imdbid=None):
        ...
        if core.sql.row_exists(TABLE, guid=guid):
            ...
            logging.info(
                "Marking {} as {} in MARKEDRESULTS.".format(guid.split("&")[0], status)
            )
            if not core.sql.update(TABLE, "status", status, "guid", guid):
                logging.info(
                    "Setting MARKEDRESULTS status of {} to {} failed.".format(
                        guid.split("&")[0], status
                    )
                )
                return False
                ...
                logging.info(
                    "Successfully marked {} as {} in MARKEDRESULTS.".format(
                        guid.split("&")[0], status
                    )
                )
                return True
            ...
            logging.info(
                "Guid {} not found in MARKEDRESULTS, creating entry.".format(
                    guid.split("&")[0]
                )
            )
            if imdbid:
                ...
                if core.sql.write(TABLE, DB_STRING):
                    logging.info(
                        "Successfully created entry in MARKEDRESULTS for {}.".format(
                            guid.split("&")[0]
                        )
                    )
                    return True
                    ...
                    logging.error(
                        "Unable to create entry in MARKEDRESULTS for {}.".format(
                            guid.split("&")[0]
                        )
                    )
                    return False
                ...
                logging.warning(
                    "Imdbid not supplied or found, unable to add entry to MARKEDRESULTS."
                )
                return False
    ...
    def movie_status(imdbid):
        ...
        cmd = (
            'SELECT DISTINCT status FROM SEARCHRESULTS WHERE imdbid="{}" AND (reject_reason IS NULL OR status <> "{}")'
            'AND type IN ("import", "{}")'.format(imdbid, "Available", '", "'.join(t))
        )

        try:
            result_status = [
                i["status"] for i in core.sql.execute([cmd]).fetchall()
            ] or []
        except Exception as e:
        ...
        for movie_status in statuses:
            for movie in results:
                ...
                if str(movie["id"]) == movie_status["tmdbid"]:
                    ...
                    movie["status"] = (
                        movie_status["status"]
                        if movie_status["status"] != "Disabled"
                        else "Finished"
                    )
    ...
    def get_stats(category=None):
        ...
        status = {"Waiting": 0, "Wanted": 0, "Found": 0, "Snatched": 0, "Finished": 0}
        ...
        if not category:
            stats["categories"] = [
                {"label": k, "value": v} for k, v in categories.items()
            ]
        stats["years"] = sorted(
            [{"year": k, "value": v} for k, v in years.items()], key=lambda k: k["year"]
        )
        stats["added_dates"] = sorted(
            [
                {"added_date": k, "value": v}
                for k, v in added_dates.items()
                if v is not None
            ],
            key=lambda k: k["added_date"],
        )
        stats["scores"] = sorted(
            [{"score": k, "value": v} for k, v in scores.items()],
