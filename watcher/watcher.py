...
if sys.version_info < (3, 0, 0):
    print(
        "Python 3.0 or newer required. Currently {}.".format(sys.version.split(" ")[0])
    )
    sys.exit(1)
...
def run():
    ...
    try:
        ...
        try:
            ...
            logging.warning(
                "Unable to set locale. Date parsing may not work correctly."
            )
            print(
                "\033[33m Unable to set locale. Date parsing may not work correctly.\033[0m"
            )

    # parse user-passed arguments
    parser = argparse.ArgumentParser(description="Watcher Server App")
    parser.add_argument(
        "-d", "--daemon", help="Run the server as a daemon.", action="store_true"
    )
    parser.add_argument("-a", "--address", help="Network address to bind to.")
    parser.add_argument("-p", "--port", help="Port to bind to.", type=int)
    parser.add_argument(
        "-b", "--browser", help="Open browser on launch.", action="store_true"
    )
    parser.add_argument(
        "--userdata", help="Userdata dir containing database, config, etc.", type=str
    )
    parser.add_argument("-c", "--conf", help="Location of config file.", type=str)
    parser.add_argument(
        "-l", "--log", help="Directory in which to create log files.", type=str
    )
    parser.add_argument("--db", help="Absolute path to database file.", type=str)
    parser.add_argument(
        "--plugins", help="Directory in which plugins are stored.", type=str
    )
    parser.add_argument(
        "--posters", help="Directory in which posters are stored.", type=str
    )
    parser.add_argument("--pid", help="Directory in which to store pid file.", type=str)
    parser.add_argument(
        "--stdout", help="Print all log messages to STDOUT.", action="store_true"
    )
    ...
    if not os.path.isfile(core.CONF_FILE):
        print(
            "\033[33m## Config file not found. Creating new basic config {}. Please review settings. \033[0m".format(
                core.CONF_FILE
            )
        )
        config.new_config()
    ...
    if passed_args.daemon:
        if core.PLATFORM == "*nix":
            ...
            menu_options = (
                ("Open Browser", None, lambda *args: webbrowser.open(core.SERVER_URL)),
            )
            systrayplugin = SysTrayPlugin(
                cherrypy.engine, "core/favicon.ico", "Watcher", menu_options
            )
    ...
    if core.CONFIG["Server"]["ssl_cert"] and core.CONFIG["Server"]["ssl_key"]:
        logger.info("SSL Certs are enabled. Server will only be accessible via https.")
        print("SSL Certs are enabled. Server will only be accessible via https.")
        ssl_conf = {
            "server.ssl_certificate": core.CONFIG["Server"]["ssl_cert"],
